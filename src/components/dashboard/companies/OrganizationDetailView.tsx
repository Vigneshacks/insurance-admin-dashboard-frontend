...
import {
  fetchCompanyDetails,
  fetchEmployees,
  deleteUser,updateEmployeeDetails
} from "../../../services/axios";
...
import UserCard from "../../../assets/cards/UserCard";
import AddUserForm from "../forms/AddUserForm";
import { RemoveUserDialog } from "../../popups/DeletePopUp";
...
import { updateUserDetails, fetchUserDetails } from "../../../services/axios";
...
const CompanyDetailsPage: React.FC = () => {
  ...
  const formatEmployeeData = (): FormattedEmployee[] => {
    ...
    return filteredEmployees.map((employee) => {
      ...
      console.log(`Employee ${employee.id} benefit_plan:`, employee.benefit_plan);
  
      // Handle insurance plans: return null if no plans, otherwise just plan_name
      const insurancePlans = Array.isArray(employee.benefit_plan) && employee.benefit_plan.length > 0
        ? employee.benefit_plan
            .map((plan) => {
              if (!plan.plan_name) {
                console.warn(`Missing plan_name for employee ${employee.id}:`, plan);
  ...
  const handleAssignInsurancePlan = async (selectedPlanIds: string[]) => {
    ...
    const loadingToast = toast.loading(`Assigning insurance plans to ${selectedEmployees.length} user(s)...`);
    ...
    try {
      ...
      const updatePromises = selectedEmployees.map(async (userId) => {
        try {
          ...
          console.log(`Updating user ${userId} in organization ${companyId}`, updateData);
  ...
  return (
    <DashboardLayout
      ...
      tag="SUPER ADMIN"
      className="font-roboto leading-[36px] text-[#1E4477]"
    >
