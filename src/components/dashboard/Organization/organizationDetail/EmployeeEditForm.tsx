...
import Header from "../../forms/components/UserEdit/FormComponents/Header";
import {
  StyledInput,
  RoleSelect,
  MultiSelectInsurance,
} from "../../forms/components/UserEdit/FormComponents";
...
const displayToApiRole: RoleConversion = (displayRole) =>
  displayRole === "user" ? "subscriber" : displayRole;
interface ApiToDisplayRole {
  (apiRole: string): string;
}

const apiToDisplayRole: ApiToDisplayRole = (apiRole) =>
  apiRole === "subscriber" ? "user" : apiRole;

const CustomUserEditForm: React.FC<CustomUserEditFormProps> = ({
  userId,
  memberGroupMasterId,
  onClose,
}) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const employeeResponse = await fetchEmployeeDetails(
          memberGroupMasterId,
          userId
        );
        ...
        const selectedPlans =
          employeeResponse.benefit_plan?.map((plan) => plan.id) || [];
        ...
        <div className="mt-auto mb-6 p-2">
          <div className="flex gap-6">
            ...
            <button
              type="submit"
              disabled={submitting}
              className={`mr-6 flex h-10 flex-1 items-center justify-center gap-2 rounded-md px-6 py-2.5 text-sm leading-5 font-medium transition-colors ${
                submitting
