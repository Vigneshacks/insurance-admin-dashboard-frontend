...
interface UserColumnVisibilityPopupProps {
  isOpen: boolean;
  onClose: () => void;
  columns: string[];
...
const UserColumnVisibilityPopup: React.FC<UserColumnVisibilityPopupProps> = ({
  isOpen,
  onClose,
  columns,
  visibleColumns,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      overlay.id = 'user-column-popup-overlay';
      ...
      const overlay = document.getElementById('user-column-popup-overlay');
      if (overlay) {
      ...
      <div className="w-[400px] rounded-lg bg-white shadow-xl pointer-events-auto">
        ...
        <div className="p-6">
          <p className="font-roboto text-[14px] leading-[20px] text-slate-700">
            Select the columns you'd like to see in your Users overview
          </p>
...
export default UserColumnVisibilityPopup;